...
if select_text == "Bunnings":
    ...
    if len(text_input) > 1:
        text_input = text_input.replace('ADOS CRC Thinner Solvent N - 1L','ADOS Thinner Solvent N - 1L')
...
if select_text == "Mitre 10":
    ...
    if len(text_input) > 1:
        ...
        replacements = {'pack of 72': 'each',
                        ' clear Not available at your chosen store.': '',
                        ' clear Not available for home delivery': '',
                        ' Check nearby stores': '',
        ...
        for i in text_input.split('each')[:-1]:
          ...
          try:
            prices.append(float(i.split('$')[2].split('each')[0].replace(' ','')+'00'))
          except:
...
if select_text == "Repco":
    ...
    if len(text_input) > 1:
        ...
        products = [i.strip() for i in text_input.split('CRC')[3::3]]
...
if select_text == "Tool Kit Depot":
    ...
    if len(text_input) > 1:
        ...
        names = [' '.join(i.split(' ')[1:]) if '$' in i else i for i in names]
        CRC_codes = [i.split('CR')[1].split(' ')[0].strip() for i in products]
        
        prices = [float(i.split('$')[1].replace(',','')) for i in raw_prices]
        first_prices = []
        for i in raw_prices:
          try:
...
if select_text == "Repco":
    ...
    if len(text_input) > 1:
        ...
        text_input = text_input.replace('ADOS Ultra High Strength Spray Adhesive Combo Pack 2x404G - 1753207', 'CRC ADOS Ultra High Strength Spray Adhesive Combo Pack 2x404G - 1753207')
...
if select_text == "Sydney Tools":
    text_input = st.text_input("Enter Sydney tools text here:")
    if len(text_input) > 1:
        text_input = text_input.replace("Product Label ","")
        test_list = text_input.split("CRC ")[::3][1:]
