...
if uploaded_file is not None:
    ...
    def round_number_up(min_max, aws, dep):
        ...
        if min_max == 'min':
            if aws == 0:
                ...
                if rounding_check(aws):
                    return aws * min_week['auto']
                else:
                    return (aws * min_week['auto']) // 1 + 1
            else:
                if rounding_check(aws):
                    return aws * min_week['else']
                else:
                    return (aws * min_week['else']) // 1 + 1
        elif min_max == 'max':
            if aws == 0:
                return 0
            elif dep == "300 AUTO AND TOOL STORAGE":
                if rounding_check(aws):
                    return aws * max_week['auto']
                else:
                    return (aws * max_week['auto']) // 1 + 1
            elif dep == "300 PAINT":
                if rounding_check(aws):
                    return aws * max_week['paint']
                else:
                    return (aws * max_week['paint']) // 1 + 1
            else:
                if rounding_check(aws):
                    return aws * max_week['else']
                else:
                    return (aws * max_week['else']) // 1 + 1
        else:
            return 'Invalid'
    ...
    for i in range(len(data)):
        department = data[1][i]
        average_weekly_sales = data[2][i]
        number = round_number_up('min', average_weekly_sales, department)
        list1.append(max(data[3][i], number))
    ...
    for i in range(len(data)):
        ...
        list1.append(max(data[4][i], number))
    ...
    for i in range(len(data)):
        if data[3][i] == "":
            data[5][i] = ""
            data[6][i] = ""
        elif data[3][i] == data[5][i] and data[4][i] == data[6][i]:
            data[5][i] = ""
            data[6][i] = ""
